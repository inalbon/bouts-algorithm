...
class wsn:
    ...
    def plotGasMap(self, ax=None, map_type='mean', timeframe=None, tsl=None, bouts_hl=0.25,
        ...
        for z in range(3):
            ...
            ax[inv_ax[z]].text(8, 2.0, 'Z = {0:.2f} m'.format(z_real[z]), rotation=0)
        ...
        if tsl is not None:
            ...
            if tsl_slice == 2:
                ax[0].plot(tsl[2], tsl[1], marker='.', markerfacecolor="tab:green", markeredgecolor="tab:green")
            if tsl_slice == 1:
                ax[1].plot(tsl[2], tsl[1], marker='.', markerfacecolor="tab:green", markeredgecolor="tab:green")
            if tsl_slice == 0:
                ax[2].plot(tsl[2], tsl[1], marker='.', markerfacecolor="tab:green", markeredgecolor="tab:green")
        print('TSL (x, y, z) ', tsl[2], tsl[1], tsl_slice)

        # display maximum estimate
        index = np.unravel_index(np.argmax(np.array(gmap), axis=None), np.array(gmap).shape)
        sensor = [1, 3, 5]
        ax[inv_ax[index[0]]].plot(sensor[index[2]], sensor[index[1]], marker='.',
