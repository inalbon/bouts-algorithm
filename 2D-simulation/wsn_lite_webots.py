...
class wsn:
        ...
        for i in range(self.nr_of_sensors):
            if mapType == 'mean':
                ...
                if mapType.endswith('freq'):
                    ...
                    val = pbc/t_diff  # bout frequency (bouts/minutes)
    ...
    def _plotGasMapSlice(self, fig, ax, gmap_slice, xlbl = True, ylbl = True, 
        ...
        if cb_lbl is not None:
            cax = plt.axes([0.7, 0.2, 0.03, 0.6])  # [left, bottom, width, height]
            cb = fig.colorbar(im_map, cax=cax) 
            cb.set_label(cb_lbl, fontsize=7)
            cb.ax.tick_params(labelsize=7)
            plt.subplots_adjust(bottom=0.2, right=0.4, top=0.9, hspace=0.9)
    ...
    def compute_bouts_amps_threshold(self, timeframe, hl=0.25, method=None, plot=None):
        ...
        for i in range(self.nr_of_sensors):
            # bout count
            filtered_bouts, amps = compute_bouts_RT(c_ppm[:, i], i, self.fs, hl=hl, ampthresh=0)
            # if plot:
