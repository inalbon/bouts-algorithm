...
W2 = wsn_lite_webots.wsn('2D_windspeed_0.1')
W3 = wsn_lite_webots.wsn('2D_windspeed_0.25')
W4 = wsn_lite_webots.wsn('2D_windspeed_0.5')
W5 = wsn_lite_webots.wsn('2D_windspeed_0.75')
W6 = wsn_lite_webots.wsn('2D_windspeed_1.0')
...
for meth in ['knee']:
    for i in range(len(list_W)):
        list_bouts_amp_thresh[i] = list_W[i].compute_bouts_amps_threshold(timeframe=[tc-delta/2, tc+delta/2],
                                                                          hl=half_life, method=meth, plot=True)

    fig, ax = plt.subplots()
    ax.plot(list_bouts_amp_thresh)
    plt.xticks(np.arange(len(list_W)), ['cluttered', 0.1, 0.25, 0.5, 0.75, 1.0])
